    ...
    return html.Div(
        ...
        children=[
            ...
            html.Div(
                id="dropdown_menu",
                children=[
                    html.H5("Select Patient's ID"),
                    dcc.Dropdown(options=id_list, value=id_list[0], id="dropdown"),
                ],
            ),
        ],
    )
...
def generate_heat_map(current_id_list, text):
    ...
    return [
        html.H6(
            f"Patiet IDs: {current_id_list[0]}-{current_id_list[-1]}  - Map of Codes"
        ),
...
def generate_one_patient_map(patient_id):
    ...
    fig.layout.width = 1000
    fig.update_layout(
        xaxis={"title": "Code Indices"},
        font=dict(family="Courier New, monospace", size=14),
        margin={"r": 0, "t": 0, "l": 0, "b": 0},
        paper_bgcolor="#daf3ff",
    )
    return [
        html.H6(f"Patient's ID: {patient_id} - Map of Codes"),
        dcc.Graph(id="heat_map", figure=fig),
    ]
...
def initialize_prediction(index):
    ...
    keys = [
        ...
        (
            "ICD10 Binary",
            parse_codes_into_str(key_list[2], index),
            parse_codes_into_str(key_list[3], index),
        ),
        (
            "ICD10 Multiclass",
            parse_codes_into_str(key_list[4], index),
            parse_codes_into_str(key_list[5], index),
        ),
    ]
    ...
    app.layout = html.Div(
        ...
        children=[
            ...
            html.Div(
                ...
                children=[
                    html.Br(),
                    html.Div(children=[generate_control_card()]),
                    html.Br(),
            ...
            html.Div(
                ...
                children=[
                    ...
                    html.Div(
                        id="patient_card",
                        children=[html.P("Patiet Map"), dcc.Graph(id="heat_map")],
                    ),
                ],
            ),
        ],
...
@app.callback(Output("dropdown_menu", "children"), Input("radio_button", "value"))
def update_dropdown(option):
    if option == 0:
        ...
        return [
            html.P("Select Range of Patient IDs"),
            dcc.Dropdown(options=values, value=values[0], id="dropdown"),
        ]
    return [
        html.P("Select Patient's ID"),
        dcc.Dropdown(options=id_list, value=id_list[0], id="dropdown"),
    ]
...
@app.callback(
    ...
    [Input("radio_button", "value"), Input("dropdown", "value")],
)
